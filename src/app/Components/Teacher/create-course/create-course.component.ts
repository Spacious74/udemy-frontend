...
export class CreateCourseComponent implements OnInit {
  ...
  saveCourseDetails(nextCallback: any) {
    ...
    if (!this.formData.description?.includes('<p>')) {
      console.log('HTML not coming from editor', this.formData);
      this.toastmsgService.showError("Error", "Course description is empty!");
      this.loading = false;
      return;
    }
    if (this.editMode) {
